...
void Camera::play()
{
	...
	cv::createTrackbar("Seuil Inférieur", "Thresholded Image", &threshold_value_1_inf, 255);
	cv::createTrackbar("Seuil Supérieur", "Thresholded Image", &threshold_value_1_sup, 255);
	...
	while(isReading)
	{
		...
		if(isReading)
		{
			...
			cv::imshow("Voies détectées", hough);
			...
			for (size_t i = 0; i < lines.size(); ) { // Note : on ne fait pas d'incrémentation ici
				float rho = lines[i][0];
			...
			cv::imshow("Voies détectées Final", hough_final);
			...
			cv::Mat kernel = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(50, 50));
			...
			for (size_t i = 0; i < contours.size(); i++) {
				...
				if ((area<10000) && (area>50)) { // Ajustez ce seuil selon la taille des voitures dans la vidéo
			...
			cv::imshow("Canny pour voiture", edges);
			cv::imshow("Canny apres fermeture", closed);
